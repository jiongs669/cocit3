...
import static com.cocit.api.APIConst.MODULE_OTHER;
...
import com.cocit.entity.security.SoftEnv;
...
public class SFTModuleEngine extends ModuleEngine {
	...
	public synchronized void setupDemsy() {
		Trans.exec(new Atom() {
			public void run() {
				...
				log.info("安装组件库到软件中......");
				entityDefEngine.setupFromPackage(soft);
				entityDefEngine.setupFromPackage(soft);
				log.infof("安装组件库 结束.");
	...
	// 用于安装平台时转换业务类为模块
	private IModule convert(IOrm orm, ITenant soft, IEntityCatalog icatalog) {
	 ...
	 * 供插件调用，用于自动生成模块文件夹
	 */
	public IModule makeModule(IOrm orm, ITenant soft, IEntityCatalog catalog) {
	 ...
	 * 供插件调用，用于自动生成模块文件夹
	 */
	public IModule makeModule(IOrm orm, ITenant soft, IEntityDefinition system) {
		if (system == null)
			return null;
		...
		if (module == null) {
			...
			module.setRefSystem(system);
			module.setPathPrefix(system.getPathPrefix());
			module.setHidden(entityDefEngine.isSlave(system));
			module.setTenantGuid(soft.getDataGuid());
	...
	private int setupDemsyModules(IOrm orm, Tenant soft) {
		...
		try {
			systems = entityDefEngine.setupSystemFromPackage(soft);
		...
		for (IEntityDefinition sys : systems) {
			...
			module.setHidden(entityDefEngine.isSlave(sys));
