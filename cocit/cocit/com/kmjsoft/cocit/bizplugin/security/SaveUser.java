...
import com.kmjsoft.cocit.entity.impl.security.WebUser;
...
import com.kmjsoft.cocit.util.HttpUtil;
...
public class SaveUser extends BasePlugin {
	...
	public void before(ActionEvent event) {
		synchronized (SaveUser.class) {
			...
			BaseUser oldUser = (BaseUser) orm.load(user.getClass(), Expr.eq(EntityConst.F_SOFT_ID, Demsy.me().getSoft()).and(Expr.eq(EntityConst.F_CODE, code)));
			...
			if (user instanceof WebUser) {
				...
				oldUser = (BaseUser) orm.load(user.getClass(), Expr.eq(EntityConst.F_SOFT_ID, Demsy.me().getSoft()).and(Expr.eq("tel", webUser.getTel())));
				if (oldUser != null && !oldUser.getId().equals(user.getId())) {
				...
				oldUser = (BaseUser) orm.load(user.getClass(), Expr.eq(EntityConst.F_SOFT_ID, Demsy.me().getSoft()).and(Expr.eq("qq", webUser.getQq())));
				if (oldUser != null && !oldUser.getId().equals(user.getId())) {
