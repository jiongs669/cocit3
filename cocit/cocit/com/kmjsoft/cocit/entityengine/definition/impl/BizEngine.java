...
public abstract class BizEngine implements IEntityDefManager {
	...
	public Nodes makeOptionNodes(IEntityColumn field, String mode, Object data, String idField) {
		...
		try {
			if (("E".equals(mode) || "M".equals(mode))) {
				...
				if (root.is((byte) 1) && entityDefManager.isSystemFK(field)) {
				...
				Object value = getFieldValue(data, field);
				if (value != null) {
					if (entityDefManager.isSystemFK(field)) {
						value = loadFieldValue(data, field);
						if (value != null) {
						...
						Option[] options = entityDefManager.getOptions(field);
