...
public abstract class ModuleEngine implements IModuleManager {
	...
	public IEntityAction getAction(IModule mdl, Serializable opID) {
		...
		if (mdl.getType() == IModule.TYPE_ENTITY) {
			if (opID instanceof String)
				...
				return ((BizEngine) entityDefManager).biz(getSystem(mdl).getId()).action(((Number) opID).longValue());
			else
				return null;
		}
			...
			if (items.size() > 0) {
				...
				if (unknownItem != null) {
					...
					while (it.hasNext()) {
						...
						if (!entityDefManager.isSystemFK(fld)) {
	...
	private void makeUpdateMenu(IModule mdl, IEntityColumn fld, Nodes root, Node item) {
		...
		if (entityDefManager.isSystemFK(fld)) {
			try {
				Class klass = entityDefManager.getGenericType(fld);
			...
			String post = entityDefManager.isV1Dic(fld) ? ".id" : "";
	...
	private Nodes makeActionNodes(IModule module, IEntityDefinition system) {
		List<? extends IEntityAction> list = ((BizEngine) entityDefManager).biz(system.getId()).actions();
	...
	public IDataSourceConfig getDataSource(Long dataSource) {
		if (this.dataSourceCache == null) {
			...
			List<INamedEntity> list = Demsy.orm().query(entityDefManager.getStaticType(BIZSYS_DEMSY_DATASOURCE));
			for (INamedEntity ele : list) {
