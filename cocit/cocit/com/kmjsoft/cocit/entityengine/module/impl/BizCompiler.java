...
public abstract class BizCompiler {
	 ...
	 *         非空——表示不可以
	 */
	protected String checkSrc(IEntityModule system) {
	 ...
	 *         非空——表示不可以
	 */
	protected String checkSrc(IEntityModule system, IEntityColumn field) {
	 ...
	 * @return 源代码片段
	 */
	protected String genSrcOfPackage(IEntityModule system) {
	 ...
	 * @return 源代码片段
	 */
	protected abstract String genSrcOfImport(IEntityModule system);
	 ...
	 *            业务系统
	 */
	protected String genSrcOfClassDeclare(IEntityModule system) {
	...
	protected String genSrcOfSystem(StringBuffer src, IEntityModule system) {
	 ...
	 *             编译业务实体类出错将抛出编译错误异常
	 */
	public synchronized List<String> compileSystem(IEntityModule system, boolean copyToClassesDir) throws DemsyException {
		if (log.isInfoEnabled()) {
			log.infof("编译<%s(%s,%s)>业务系统......", system.getName(), system.getCode(), system.getId());
		}

		List<IEntityModule> collectedSystems = new LinkedList();
		...
		if (log.isInfoEnabled()) {
			...
			for (IEntityModule sys : collectedSystems) {
		...
		try {
			...
			for (int i = collectedSystems.size() - 1; i >= 0; i--) {
				IEntityModule sys = collectedSystems.get(i);
	 ...
	 *            编译的目标业务系统
	 */
	private void collectBizSystems(List collectedSystems, IEntityModule targetSystem) {
