...
class CacheUI {
	...
	CacheUI(UiEngine engine, Long pageID) {
		if (pageID != null) {
			page = (IPage) Demsy.orm().load(entityModuleManager.getStaticType(BIZSYS_UIUDF_PAGE), pageID);

			if (page != null) {
				blocks = Demsy.orm().query(entityModuleManager.getStaticType(BIZSYS_UIUDF_PAGE_BLOCK), Expr.eq(F_UI_PAGE, page).and(Expr.eq(F_DISABLED, false)).addAsc(F_ORDER_BY));
