...
import com.kmjsoft.cocit.entity.impl.module.ActionDefinition;
import com.kmjsoft.cocit.entity.impl.module.EntityAction;
import com.kmjsoft.cocit.entity.impl.module.EntityCatalog;
...
import com.kmjsoft.cocit.entity.impl.module.SystemDataType;
...
public class SFTBizEngine extends BizEngine {
	...
	@Override
	public String getPackageOfAutoSystem(IEntityModule system) {
		return "com.kmetop.demsy.comlib.impl.sft.dynamic.system";
	}

	public String getSimpleClassName(IEntityModule system) {
		return "SFTSystem_" + Long.toHexString(system.getId());
	}

	@Override
	public String getExtendClassName(IEntityModule system) {
	...
	@Override
	public String getExtendSimpleClassName(IEntityModule system) {
		String extClass = system.getExtendClass();
		if (Str.isEmpty(extClass)) {
	...
	@Override
	public IEntityModule setupSystemFromClass(ITenant soft, Class klass) throws DemsyException {
		return setupSystemFromClass(soft.getDataGuid(), klass, true);
	}

	private IEntityModule setupSystemFromClass(String soft, Class klass, boolean autoUpdate) throws DemsyException {
		...
		EntityModule system = (EntityModule) orm.load(EntityModule.class, CndExpr.eq(F_CODE, sysCode));
	...
	public void parseSystemByAnnotation(Class klass, IEntityModule system) {
		ExtOrm orm = orm();
	...
	private EntityModule parseSystem(ExtOrm orm, String soft, Class klass, EntityModule system, CocEntity sysann, int sysOrder, String code) {
		if (system == null) {
			system = new EntityModule();
		}
	...
	private EntityColumn parseBizField(ExtOrm orm, String soft, Mirror me, IEntityModule system, EntityColumnGroup group, CocColumn fldann, int gridOrder, int fieldOrder, Map fieldLibs) {
		...
		if (f == null || f.getAnnotation(CocColumn.class) == null) {
			...
			if (!Str.isEmpty(fldann.fkEntity())) {
				field.setRefrenceSystem((EntityModule) setupSystemFromClass(soft, getStaticType(fldann.fkEntity()), false));
	...
	private EntityColumnGroup parseBizGroup(ExtOrm orm, String soft, IEntityModule system, CocGroup grpann, int groupOrder) {
		...
		group.setSystem((EntityModule) system);
	...
	private EntityAction parseBizAction(ExtOrm orm, String soft, IEntityModule system, CocAction actann, int actionOrder) {
	...
	private List<EntityAction> parseBizAction(ExtOrm orm, String soft, IEntityModule system, String jsonData, int actionOrder) {
	...
	private void parseBizAction(List<EntityAction> list, ExtOrm orm, String soft, IEntityModule system, EntityAction newAction, EntityAction parentAction, int actionOrder) {
	...
	public IEntityColumn parseBizField(String soft, Mirror entityObj, String fldProp, IEntityModule bzSystem, IEntityColumn oldBzField) {
		Map<String, IFieldDataType> fieldLibs = this.getFieldTypes();
		...
		newBzField.setSystem((EntityModule) bzSystem);
		...
		try {
			...
			if (annBzFld != null && !Str.isEmpty(annBzFld.fkEntity())) {
				newBzField.setRefrenceSystem((EntityModule) setupSystemFromClass(soft, getStaticType(annBzFld.fkEntity()), false));
				fieldLib = fieldLibs.get("System");
			} else if (manyToOne != null) {
				newBzField.setRefrenceSystem((EntityModule) setupSystemFromClass(soft, type, false));
				fieldLib = fieldLibs.get("System");
				...
				if (genericTypes != null && genericTypes.length > 0) {
					newBzField.setRefrenceSystem((EntityModule) setupSystemFromClass(soft, genericTypes[0], false));
				}
			...
			if (dot > 0) {
				...
				EntityModule refSystem = parentField.getRefrenceSystem();
