...
public class SFTModuleEngine extends ModuleEngine {
	...
	public synchronized void setupDemsy() {
		Trans.exec(new Atom() {
			public void run() {
				...
				log.info("安装组件库到软件中......");
				entityModuleManager.setupFromPackage(soft);
				entityModuleManager.setupFromPackage(soft);
				log.infof("安装组件库 结束.");
	...
	// 用于安装平台时转换业务类为模块
	private IFunMenu convert(ExtOrm orm, ITenant soft, IEntityCatalog icatalog) {
		...
		FunMenu funMenu = (FunMenu) orm.load(FunMenu.class, CndExpr.eq(F_CODE, catalog.getCode()));
		if (funMenu == null) {
			funMenu = new FunMenu();
		}

		funMenu.setName(catalog.getName());
		funMenu.setType(IFunMenu.TYPE_FOLDER);
		funMenu.setParent((FunMenu) convert(orm, soft, catalog.getParent()));
		funMenu.setTenantOwnerGuid(soft.getDataGuid());
		funMenu.setSerialNumber(catalog.getSerialNumber());
		funMenu.setCode(catalog.getCode());
		funMenu.setBuildin(true);

		Obj.makeSetupGuid(funMenu, funMenu.getCode());
	 ...
	 * 供插件调用，用于自动生成模块文件夹
	 */
	public IFunMenu makeModule(ExtOrm orm, ITenant soft, IEntityCatalog catalog) {
		if (catalog == null || (Str.isEmpty(catalog.getDataGuid()) && Str.isEmpty(catalog.getCode())))
			return null;

		FunMenu funMenu = (FunMenu) orm.load(FunMenu.class, CndExpr.eq(F_CODE, catalog.getDataGuid()));
		if (funMenu == null && !Str.isEmpty(catalog.getCode())) {
			funMenu = (FunMenu) orm.load(FunMenu.class, CndExpr.eq(F_CODE, catalog.getCode()));
		}
		if (funMenu == null) {
			funMenu = new FunMenu();
			funMenu.setName(catalog.getName());
			funMenu.setCode(catalog.getDataGuid());
			funMenu.setType(IFunMenu.TYPE_FOLDER);
			funMenu.setParent((FunMenu) makeModule(orm, soft, (IEntityCatalog) catalog.getParent()));
			funMenu.setTenantOwnerGuid(soft.getDataGuid());
	 ...
	 * 供插件调用，用于自动生成模块文件夹
	 */
	public IFunMenu makeModule(ExtOrm orm, ITenant soft, IEntityModule system) {
		if (system == null)
			return null;

		FunMenu funMenu = (FunMenu) orm.load(FunMenu.class, CndExpr.eq(F_CODE, system.getDataGuid()));
		if (funMenu == null) {
			funMenu = new FunMenu();
			funMenu.setName(system.getName());
			funMenu.setCode(system.getDataGuid());
			funMenu.setType(IFunMenu.TYPE_ENTITY);
			funMenu.setParent((FunMenu) this.makeModule(orm, soft, system.getCatalog()));
			funMenu.setRefSystem(system);
			funMenu.setPathPrefix(system.getPathPrefix());
			funMenu.setHidden(entityModuleManager.isSlave(system));
			funMenu.setTenantOwnerGuid(soft.getDataGuid());
	...
	private int setupDemsyModules(ExtOrm orm, Tenant soft) {
		...
		FunMenu otherModule = (FunMenu) orm.load(FunMenu.class, CndExpr.eq(F_CODE, moduleFolder).and(CndExpr.eq(F_TENANT_OWNER_GUID, soft)));

		List<IEntityModule> systems;
		try {
			systems = entityModuleManager.setupSystemFromPackage(soft);
		...
		for (IEntityModule sys : systems) {
			...
			FunMenu funMenu = (FunMenu) orm.load(FunMenu.class, CndExpr.eq(F_CODE, code));
			if (funMenu == null) {
				funMenu = new FunMenu();
			}
			funMenu.setName(sys.getName());
			funMenu.setSerialNumber(sys.getSerialNumber());
			funMenu.setCode(code);
			funMenu.setType(IFunMenu.TYPE_ENTITY);
			funMenu.setRefSystem(sys);
			funMenu.setPathPrefix(sys.getPathPrefix());
			funMenu.setHidden(entityModuleManager.isSlave(sys));
			funMenu.setStatusCode(IDataEntity.STATUS_CODE_BUILDIN);
			funMenu.setTenantOwnerGuid(soft.getDataGuid());

			FunMenu parentModule = (FunMenu) convert(orm, soft, (EntityCatalog) sys.getCatalog());
			if (parentModule == null) {
				if (otherModule == null) {
					otherModule = new FunMenu();
					otherModule.setName("==其他模块==");
					otherModule.setCode(moduleFolder);
					otherModule.setType(IFunMenu.TYPE_FOLDER);
			...
			funMenu.setParent(parentModule == null ? otherModule : parentModule);
			Obj.makeSetupGuid(funMenu, code);
