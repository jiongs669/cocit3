...
public class UiEngine implements IUIEngine, MvcConst {
	...
	@Override
	public UIBizModule makeModuleView(IFunMenu mdl, String gridColumns, String idField) throws DemsyException {
	...
	@Override
	public UIBizSystem makeSystemView(IFunMenu mdl, String gridColumns, String idField) throws DemsyException {
	...
	@Override
	public UIBizGridModel makeSystemGridView(IFunMenu mdl, String colNames, String idField, boolean existNaviTree) throws DemsyException {
		...
		if (grid == null) {
			...
			IEntityModule sys = funMenuManager.getSystem(mdl);
			List<? extends IEntityColumn> flds = entityModuleManager.getFieldsOfGrid(sys, colNames);
			int count = 1;
			int width = 0;
			...
			for (IEntityColumn fld : flds) {
				...
				if (entityModuleManager.isString(fld) || entityModuleManager.isRichText(fld) || entityModuleManager.isSystemFK(fld)) {
					col.setString(true);
				}

				col.setLabel(fld.getName());
				col.setName(fld.getCode());
				col.setPropName(entityModuleManager.getPropName(fld));
				...
				if (entityModuleManager.isNumber(field) && options.length == 0) {
					col.setAlign(Align.RIGHT);
					col.setSearchType("number");
				} else if (entityModuleManager.isBoolean(field)) {
					col.setAlign(Align.CENTER);
				} else if (entityModuleManager.isDate(field)) {
					col.setSearchType("date");
				}

				// 计算字段宽度
				int w = entityModuleManager.getGridWidth(field);
				col.setWidth(w);
				width += w;
	...
	@Override
	public UIBizNaviModel makeSystemNaviView(IFunMenu mdl) throws DemsyException {
		...
		if (navi == null) {
			...
			if (entityModuleManager.getFieldsOfNavi(funMenuManager.getSystem(mdl)).size() == 0) {
				navi.setWidth(0);
			} else {
				navi.setWidth(300);
	...
	public UIBizMenuModel<UIToolbarMenu> makeSystemActionView(IFunMenu funMenu) throws DemsyException {
		...
		UIToolbarMenu menu = cached("ToolbarMenu", funMenu.getId());
		if (menu == null) {
			menu = cache("ToolbarMenu", new UIToolbarMenu(globalVariables, funMenu.getId()));

			// 设置模版环境变量——遗留系统中的图标路径
			menu.set("moduleID", funMenu.getId());
			menu.set("type", URL_PREFIX_BZ);

			// TODO: 通过配置指定数据类型，如果数据类型为null表示不采用AJAX方式加载数据
			menu.setDataUrl(contextPath(URL_BZMENU, URL_PREFIX_AJAX + funMenu.getId()));
	...
	public UIBizFormModel makeSystemFormView(IFunMenu funMenu, IEntityAction entityAction, Object data) throws DemsyException {
		...
		UIBizForm form = new UIBizForm(globalVariables, funMenu.getId());
		form.setDataType(MvcConst.DATA_HTML);

		form.set("moduleID", funMenu.getId());

		IEntityModule sys = funMenuManager.getSystem(funMenu);

		// 获取运行时自定义字段
		List<IEntityColumn> customFields = new LinkedList();
		Map<String, IEntityColumn> customFieldsMap = new HashMap();
		List<IEntityColumn> fkFieldsOfRuntimeCustom = (List<IEntityColumn>) entityModuleManager.getFieldsOfSystemFK(sys, IRuntimeField.class);
		...
		for (IEntityColumn fld : fkFieldsOfRuntimeCustom) {
			...
			List<? extends IEntityColumn> flds = entityModuleManager.makeFields(custom);
			if (flds != null) {
		...
		List<? extends IEntityColumnGroup> bizGroups = entityModuleManager.getFieldGroups(sys);
		...
		for (IEntityColumnGroup bizGroup : bizGroups) {
			...
			uiGroup.setMode(entityModuleManager.getMode(bizGroup, entityAction));

			List<? extends IEntityColumn> groupFields = entityModuleManager.getFieldsOfEnabled(bizGroup);
			for (IEntityColumn bzField : groupFields) {
				try {

					// 用运行时自定义字段覆盖物理字段
					String bzFieldPropName = entityModuleManager.getPropName(bzField);
					...
					UIBizFld uiField = this.convertFld(funMenu, form, bzField, entityAction, null, data, String.class.equals(entityModuleManager.getType(bzField)) ? F_DATA_GUID : F_ID);
					if (uiField != null) {
		...
		uiGroup.setMode("S");
		Map<Long, IFieldDataType> types = entityModuleManager.getFieldTypesById();
		...
		for (IEntityColumn bzField : customFields) {
			try {
				...
				UIBizFld uiField = this.convertFld(funMenu, form, bzField, entityAction, null, data, F_ID);
				if (uiField != null) {
					uiGroup.addChild(uiField, data);
					uiFieldMap.put(uiField.getPropName(), uiField);
				}
	...
	private UIBizFld convertFld(IFunMenu funMenu, UIBizForm form, IEntityColumn bzField, IEntityAction entityAction, String defaultMode, Object data, String idField) {

		String[] cascade = entityModuleManager.getCascadeMode(bzField, data);
		String mode = entityModuleManager.getMode(bzField, entityAction, false, defaultMode);
		if (entityModuleManager.getModeValue(mode) < entityModuleManager.getModeValue(cascade[1])) {
			mode = entityModuleManager.getUiMode(cascade[1]);
		}
		// log.tracef("bzField:%s, mode:%s, defaultMode:%s", bzField.getName(),
		// mode, defaultMode);

		String prop = entityModuleManager.getPropName(bzField);
		...
		String namePost = "";
		if (entityModuleManager.isV1Dic(bzField)) {// 遗留系统字典
			namePost = ".id";
		}

		Class bzFieldType = entityModuleManager.getType(bzField);
		// ui model
		if (entityModuleManager.isBoolean(bzField)) {// boolean
			uiField = new UIBoolFld(null, bzField.getId()).setOptionNode(entityModuleManager.makeOptionNodes(bzField, mode, data, idField));
		} else if (entityModuleManager.isUpload(bzField)) {// upload
			uiField = new UIUploadFld(null, bzField.getId()).setUploadUrl(contextPath(URL_UPLOAD, funMenu.getId(), bzField.getId())).setUploadType(bzField.getUploadType()).setContext(form.getContext());
			isUpload = true;
		} else if (entityModuleManager.isRichText(bzField)) {// richtext
			uiField = new UIRichTextFld(null, bzField.getId());
			isRichText = true;
		} else if (entityModuleManager.isText(bzField)) {// text
			uiField = new UITextFld(null, bzField.getId());
		} else if (entityModuleManager.getOptions(bzField).length > 0) {// auto dic
			if (entityModuleManager.isManyToMany(bzField)) {
				uiField = new UIDicManyFld(null, bzField.getId()).setOptionNode(entityModuleManager.makeOptionNodes(bzField, mode, data, idField));
				form.setHasMultiSelect(true);
			} else {
				uiField = new UIDicFld(null, bzField.getId()).setOptionNode(entityModuleManager.makeOptionNodes(bzField, mode, data, idField));
			}
		} else if (entityModuleManager.isDate(bzField)) {// date
			uiField = new UIDateFld(null, bzField.getId());
			isDate = true;
		} else if (entityModuleManager.isNumber(bzField)) {// number
			uiField = new UINumFld(null, bzField.getId());
		} else if (entityModuleManager.isString(bzField)) {// string
			uiField = new UIStrFld(null, bzField.getId());
			if (bzField.isPassword()) {
				((UIStrFld) uiField).setInputType("password");
			}
		} else if (entityModuleManager.isSystemFK(bzField)) {// system refrence
			Class type = null;
			if (entityModuleManager.isManyToMany(bzField)) {
				namePost = ".id";
				...
				try {
					type = Cls.getType(entityModuleManager.getType(bzField.getSystem()), entityModuleManager.getPropName(bzField));
					if (type.isAssignableFrom(entityModuleManager.getType(bzField.getRefrenceSystem()))) {
						namePost = ".id";
					}
				} catch (Throwable e) {
			...
			Nodes options = entityModuleManager.makeOptionNodes(bzField, mode, data, idField);
			uiField.setOptionNode(options);
			if (options.is((byte) 1)) {
				isCombobox = true;
				IFunMenu refModule = funMenuManager.getModule(funMenu.getTenantOwnerGuid(), bzField.getRefrenceSystem());
				List<String> rules = entityModuleManager.makeCascadeExpr(data, bzField, entityAction.getMode());
				...
				if (type.equals(String.class)) {
					uiField.setKeyProp(EntityConst.F_DATA_GUID);
					url += "&idField=" + EntityConst.F_DATA_GUID;
				}
			...
			uiField = new UIStrFld(null, bzField.getId());
		} else if (entityModuleManager.isFieldRef(bzField)) {// iglore field refrence
			return null;
		} else if (entityModuleManager.isSubSystem(bzField)) {
			uiField = new UISubSystemFld(null, bzField.getId()).setUploadUrl(contextPath(URL_UPLOAD, funMenu.getId(), bzField.getId())).setUploadType(bzField.getUploadType()).setContext(form.getContext());
			UISubSystemFld uiSubSysFld = (UISubSystemFld) uiField;

			IEntityModule refSystem = bzField.getRefrenceSystem();
			String[] refProps = Str.toArray(bzField.getRefrenceFields());

			if (entityModuleManager.isFakeSubSystem(bzField)) {
				uiSubSysFld.setFake(true);
			}
			if (entityModuleManager.isMultiUpload(bzField)) {
				CocColumn ann = (CocColumn) bzFieldType.getAnnotation(CocColumn.class);
				refSystem = entityModuleManager.getSystem(ann.fkEntity());
				refProps = Str.toArray(ann.fkField());
			}

			Map<String, IEntityColumn> refFields = entityModuleManager.getFieldsMap(refSystem);
			for (String refProp : refProps) {
				IEntityColumn refFld = refFields.get(refProp);
				if (refFld != null) {
					UIBizFld uiRefFld = this.convertFld(funMenu, form, refFld, entityAction, defaultMode, null, null);

					uiRefFld.setMode("E");
					if (entityModuleManager.isNumber(refFld) && !entityModuleManager.isInteger(refFld)) {
						uiRefFld.setPattern("#.00");
					}
					if (entityModuleManager.isSystemFK(refFld))
			...
			if (!"N".equals(mode))
				this.convertSubFields(funMenu, form, entityAction, data, bzField, uiField);
		}
	...
	private void convertSubFields(IFunMenu funMenu, UIBizForm form, IEntityAction entityAction, Object data, IEntityColumn bzfld, UIBizFld uifld) {
		Class proptype = entityModuleManager.getType(bzfld);
		...
		long count = 1;
		Class systype = entityModuleManager.getType(bzfld.getSystem());
		...
		if (subanns != null && subanns.length > 0) {
			for (CocField2 ann : subanns) {
				IEntityColumn fld = ((BizEngine) entityModuleManager).parseBizField(funMenu.getTenantOwnerGuid(), propme, ann.propName(), bzfld.getSystem(), null);
				fld.setId(count++);
			...
			for (Field f : fields) {
				IEntityColumn fld = ((BizEngine) entityModuleManager).parseBizField(funMenu.getTenantOwnerGuid(), propme, f.getName(), bzfld.getSystem(), null);
				fld.setId(count++);
				fld.setPropName(prop + "." + fld.getPropName());
		...
		for (IEntityColumn bzChild : bzsubflds) {
			UIBizFld subUiFld = this.convertFld(funMenu, form, bzChild, entityAction, uifld.getMode(), data, EntityConst.F_DATA_GUID);
	...
	public IPage loadIndexPage() {
		return (IPage) Demsy.orm().load(entityModuleManager.getStaticType(BIZSYS_UIUDF_PAGE), Expr.eq(F_USAGE, IPage.USAGE_IDX).and(Expr.eq(F_TENANT_OWNER_GUID, Demsy.me().getTenant())).setFieldRexpr("id", true));
	}
	...
	public IPageBlock loadPageBlock(Long blockID) {
		...
		if (block == null) {
			return (IPageBlock) Demsy.orm().load(entityModuleManager.getStaticType(BIZSYS_UIUDF_PAGE_BLOCK), Expr.eq(F_ID, blockID));
		}
		return block;
	}
	...
	private void loadViewComponents() {
		if (this.viewComponentList.size() == 0 || !Demsy.appconfig.isProductMode()) {
			viewComponentList = Demsy.orm().query(entityModuleManager.getStaticType(BIZSYS_DEMSY_LIB_UIMODEL), Expr.asc(F_ORDER_BY));
	...
	public UIPageView makePageView(IPage page, IPageBlock dynamicBlock, Long dynamicModuleID, Long dynamicDataID) {
		...
		if (dynamicModuleID != null && dynamicModuleID > 0) {
			if (dynamicDataID != null && dynamicDataID >= 0) {
				pathModule = funMenuManager.getModule(dynamicModuleID);
				IEntityModule pathSystem = funMenuManager.getSystem(pathModule);
				pathData = Demsy.orm().load(entityModuleManager.getType(pathSystem), Expr.eq(F_ID, dynamicDataID));
			}
		}
	...
	private UIBlockViewModel makeBlockViewWithChilren(Map<Long, UIBlockViewModel> cachedBlockViews, Map<Long, IPageBlock> cachedBlocks, IPageBlock defaultParentBlock, IPageBlock pageBlock, Long dynamicModuleID, Long dynamicDataID,
			IFunMenu pathModule, Object pathData) {
		...
		List<IPageBlock> children = Demsy.orm().query(entityModuleManager.getStaticType(BIZSYS_UIUDF_PAGE_BLOCK), Expr.eq(F_PARENT, pageBlock).and(Expr.eq(F_DISABLED, false)).addAsc(F_ORDER_BY));
	...
	private UIBlockViewModel makeBlockView(UIPageView pageView, UIBlockViewModel parentBlockView, IPageBlock block, Long dynamicModuleID, Long dynamicDataID, IFunMenu pathModule, Object pathData) {
	...
	private void addViewComponent(Node fnode, IUIViewComponent viewType, IEntityColumn field, String var) {
		...
		if (entityModuleManager.isImage(field)) {
		...
		} else if (entityModuleManager.isNumber(field) && !entityModuleManager.isInteger(field)) {
	...
	private void addViewComponent(Nodes root, IFunMenu folder, Node folderNode, IUIViewComponent viewController) {
		for (Node moduleNode : folderNode.getChildren()) {
			IFunMenu funMenu = (IFunMenu) moduleNode.get("moduleID");
			...
			Node fnode = root.addNode("bzmodule" + folder.getId(), "bzmodule" + funMenu.getId()).setName(funMenu.getName());
			...
			fnode.set("moduleGuid", funMenu.getDataGuid());
			fnode.set("expr", "");
		}
	}
	...
	public Nodes makeNodesOfViewCompnents(Long blockID) {
		...
		if (blockID != null) {
			...
			if (blockContext != null) {
				IEntityModule catalogSystem = blockContext.getCatalogSystem();
				IEntityModule system = blockContext.getSystem();
				if (catalogSystem != null) {
					...
					List<? extends IEntityColumn> fields = entityModuleManager.getFieldsOfEnabled(catalogSystem);
		...
		Nodes moduleNodes = funMenuManager.makeNodesByModule(Demsy.me().getTenant(), SecurityManager.ROLE_ADMIN_ROOT);
		for (Node folderNode : moduleNodes.getChildren()) {
			IFunMenu folder = (IFunMenu) folderNode.get("moduleID");
	...
	private void loadUiStyle() {
		if (this.styleCache.size() == 0 || !appconfig.isProductMode()) {
			List<IStyle> uiStyleList = Demsy.orm().query(entityModuleManager.getStaticType(BIZSYS_UIUDF_STYLE), Expr.asc(F_ORDER_BY));
	...
	public void addClickNum(ExtOrm orm, IStatistic obj) {
		funMenuManager.increase(orm, obj, "clickNum");
	}

	public void addCommentNum(ExtOrm orm, IStatistic obj) {
		funMenuManager.increase(orm, obj, "commentNum");
	}
