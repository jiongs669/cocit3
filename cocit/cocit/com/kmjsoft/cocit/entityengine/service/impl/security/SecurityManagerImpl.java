package com.kmjsoft.cocit.entityengine.service.impl.security;
...
import com.kmjsoft.cocit.entityengine.service.UserService;
...
import com.kmjsoft.cocit.util.Json;
...
public class SecurityManagerImpl implements SecurityManager {
	...
	private Map<String, List<RuntimePermission>> permissionsModuleMap;

	private Map<String, List<RuntimePermission>> permissionsTableMap;

	// 临时权限：通常有运行时的程序动态授予的权限。
	private Map<String, RuntimePermission> tempPermissionsMap;
	 ...
	 * @return
	 */
	private List<RuntimePermission> getPermissions(TableService table) {
	 ...
	 * @return
	 */
	private List<RuntimePermission> getPermissions(ModuleService module) {
		if (permissionsModuleMap.size() == 0 && this.permissionsTableMap.size() == 0)
			this.loadPermissions();
	...
	private void loadPermissions() {
		List<Permission> entityList = orm.query(Permission.class, Expr.eq("softID", softService.getID()));
		TableService tmpTable;
		String str;
		List<RuntimePermission> runtimePermissions;
	...
	public boolean check(UserService user, ModuleService module) {
		...
		// 获取模块授权列表
		List<RuntimePermission> items = this.getPermissions(module);
		if (items == null)
			return false;

		for (RuntimePermission perm : items) {
	...
	public boolean check(UserService user, TableService table, String opMode, Long... data) {
		...
		// 获取模块授权列表
		List<RuntimePermission> items = this.getPermissions(table);
		if (items == null)
			return false;
	...
	public void authorize(String key, byte userRole, String tableCode) {
		...
		RuntimePermission p = RuntimePermission.make(userRole, tableID);
