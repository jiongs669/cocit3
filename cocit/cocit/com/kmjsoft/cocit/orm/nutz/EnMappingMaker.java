...
import com.kmjsoft.cocit.orm.ann.Klass;
import com.kmjsoft.cocit.orm.ann.Prop;
import com.kmjsoft.cocit.orm.generator.INamingStrategy;
import com.kmjsoft.cocit.orm.generator.impl.TableEntityIdGenerator;
import com.kmjsoft.cocit.orm.mapping.EnColumnMapping;
...
public class EnMappingMaker implements EntityMaker {
	...
	public EnMapping<?> make(EnMapping<?> parent, Class<?> type) {
		...
		log.debugf("解析%s实体......[parentEntity=%s]", enInfo, parent == null ? "NULL" : Cls.getDisplayName(parent.getType()));
			...
			try {
				...
				if (link != null) {
					...
					if (col == null || Str.isEmpty(col.name())) {
						...
						String propertyTableName = Str.unqualify(klassName(tableNameClass, tableNameClass.getSimpleName()));
						...
						if (encoding) {
							if (null == annCol || Str.isEmpty(annCol.name())) {
								...
								referencedColumnName = namingStrategy.logicalColumnName(annCol.name(), targetPk.getName());
							}

							columnName = namingStrategy.foreignKeyColumnName(propertyName, propertyEntityName, propertyTableName, referencedColumnName);
						} else {
							if (null == annCol || Str.isEmpty(annCol.name())) {
						...
						log.tracef("计算 FK 字段： result=%s, propertyName=%s, propertyEntityName=%s, propertyTableName=%s, referencedColumnName=%s", columnName, propertyName, propertyEntityName, propertyTableName, referencedColumnName);
					} else {
						Field targetPk = idField(Mirror.me(link.getTargetClass()));
						ef.setColumnDefinition(targetPk.getType().getSimpleName().toLowerCase());
						columnName = col.name();
	...
	private EnColumnMappingImpl evalField(Mirror<?> mirror, EnMappingImpl<?> entity, Field field, boolean encoding) {
		...
		if (encoding) {
			if (null == column || Str.isBlank(column.name())) {
				...
				ef.setColumnName(columnName);
				log.tracef("计算 PROPERTY 字段：result=%s, propertyName=%s, propAnnValue=%s", columnName, field.getName(), name);
			} else {
				String name = column.name();
				String columnName = namingStrategy.columnName(name);
				ef.setColumnName(columnName);
				log.tracef("计算 COLUMN 字段：result=%s, propertyName=%s, columnAnnName=%s", columnName, field.getName(), name);
			}
		} else {
			if (null == column || Str.isBlank(column.name())) {
				String name = propName(field, field.getName());
		...
		try {
			...
			if (null != many) {
				...
				if (Str.isEmpty(many.mappedBy())) {
					throw Ex.throwEx("<%s.%s>@OneToMany mappedBy is mustable.", mirror.getType().getName(), field.getName());
				} else {
			...
			if (manyToMany != null) {
				/*
				 * targetXXX 和 selfXXX 用于创建多对多Link对象。 ownerXXX 和 associatedXXX 用于生成中间表名称
				 */
				...
				String from = ns.foreignKeyColumnName(selfPropertyName, selfPropertyEntityName, selfPropertyTableName, fromReferencedColumnName);
				...
				String to = ns.foreignKeyColumnName(targetPropertyName, targetPropertyEntityName, targetPropertyTableName, toReferencedColumnName);

				// 计算中间表名称
				String relation = ns.collectionTableName(ownerEntity, ownerEntityTable, associatedEntity, associatedEntityTable, propertyName);

				if (log.isTraceEnabled()) {
					log.tracef("计算 MANY2MANY 主实体： [\nownerEntity=%s\nownerEntityTable=%s\nownerClass=%s\n]", ownerEntity, ownerEntityTable, ownerClass.getName());
					log.tracef("计算 MANY2MANY 关联实体：[\nassociatedEntity=%s\nassociatedEntityTable=%s\nassociatedClass=%s\n]", associatedEntity, associatedEntityTable, associatedClass.getName());
					log.tracef("计算 MANY2MANY 本POJO： [\nfrom=%s\nselfPk=%s\npropertyName=%s\npropertyEntityName=%s\npropertyTableName=%s\nreferencedColumnName=%s\n]", from, selfPk.getName(), selfPropertyName, selfPropertyEntityName,
							selfPropertyTableName, fromReferencedColumnName);
					log.tracef("计算 MANY2MANY 目标POJO： [\nto=%s\ntargetPk=%s\npropertyName=%s\npropertyEntityName=%s\npropertyTableName=%s\nreferencedColumnName=%s\n]", to, targetPk.getName(), targetPropertyName, targetPropertyEntityName,
							targetPropertyTableName, toReferencedColumnName);
					log.tracef("计算 MANY2MANY 中间表： [\nrelation=%s\nownerEntity=%s\nownerEntityTable=%s\nassociatedEntity=%s\nassociatedEntityTable=%s\npropertyName=%s\n]", relation, ownerEntity, ownerEntityTable, associatedEntity,
							associatedEntityTable, propertyName);
					log.tracef("计算 MANY2MANY 映射：[\nselfClass=%s\nselfField=%s\ntargetClass=%s\nselfPk=%s\ntargetPk=%s\nrelation=%s\nfrom=%s\nto=%s\n]", mirror.getType().getName(), field.getName(), targetClass.getName(), selfPk, targetPk, relation,
							from, to);
				}

				// 创建多对多Link对象
				Link link = Link.getLinkForManyMany(mirror, field, targetClass, selfPk, targetPk, null, relation, from, to);
			...
			throw Ex.throwEx("Fail to eval linked field '%s' of class[%s] for the reason '%s'", field.getName(), mirror.getType().getName(), Ex.msg(e));
		}
		return null;
	}
	...
	private EntityName evalTableName(INamingStrategy ns, Class<?> type) {
		...
		if (table != null && !Str.isBlank(table.name())) {
			...
			log.tracef("计算 ENTITY 表：result=%s, entityClass=%s, tableAnnName=%s", tableName, type.getName(), table.name());
