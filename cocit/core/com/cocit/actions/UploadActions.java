...
import com.cocit.api.web.IUploadInfo;
...
import com.cocit.modules.ckfinder.CKFinder;
...
public class UploadActions implements MvcConst {
	...
	private IUploadInfo save(TempFile model, String moduleId, String fieldId) throws DemsyException, IOException {
		 ...
		 * 保存文件信息到数据库
		 */
		IUploadInfo info = (IUploadInfo) Mirror.me(Demsy.entityDefEngine.getStaticType(APIConst.BIZSYS_ADMIN_UPLOAD)).born();
		try {
			Demsy me = Demsy.me();
			if (me.getSoft() != null)
			...
			if (!Str.isEmpty(fieldId)) {
				try {
					info.setBzfield(Demsy.entityDefEngine.getField(Long.parseLong(fieldId)));
				} catch (Throwable e) {
				}
			}
