...
import static com.cocit.api.APIConst.BIZSYS_DEMSY_LIB_FIELD;
...
import static com.cocit.api.APIConst.F_GRID_ORDER;
...
import static com.cocit.api.APIConst.F_UPDATED;
import static com.cocit.api.APIConst.F_VERSION;
...
import com.cocit.api.IEntityDefEngine;
...
import com.cocit.api.entitydef.field.IExtField;
...
import com.cocit.security.ILogin;
...
public abstract class BizEngine implements IEntityDefEngine {
	...
	public List<IEntityDefinition> setupSystemFromPackage(ITenant soft) throws DemsyException {
		...
		while (types.hasNext()) {
			IEntityDefinition sys = setupSystemFromClass(soft, types.next());
			if (sys != null) {
				ret.add(sys);
			}
		}
	 ...
	 * @return 包名称
	 */
	public abstract String getPackageOfAutoSystem(IEntityDefinition system);
	 ...
	 * @return 扩展类全称
	 */
	public abstract String getExtendSimpleClassName(IEntityDefinition system);
	 ...
	 * @return 类简称
	 */
	public abstract String getSimpleClassName(IEntityDefinition system);
	 ...
	 * @return
	 */
	public abstract String getTargetPropName(IFieldDefinition field);

	public abstract IFieldDefinition parseBizField(String tenantGuid, Mirror entityObj, String prop, IEntityDefinition system, IFieldDefinition field);

	@Override
	public int getPrecision(IFieldDefinition fld) {
		Integer p = fld.getPrecision();
		if (p == null || p == 0) {
			if (isString(fld))
				return APIConst.DEFAULT_PRECISION;
			else
				return 0;
		}
	...
	public boolean isSystemFK(IFieldDefinition fld) {
	...
	public boolean isFieldRef(IFieldDefinition fld) {
	...
	@Override
	public boolean isEnabled(IFieldDefinition fld) {
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfEnabled(IEntityDefinition system) {
		List<? extends IFieldDefinition> list = this.getFields(system);

		List<IFieldDefinition> ret = new LinkedList();
		for (IFieldDefinition f : list) {
			if (this.isEnabled(f))
				ret.add(f);
		}
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfEnabled(IFieldGroup group) {
		List<? extends IFieldDefinition> list = this.getFields(group);
	...
	@Override
	public IFieldDefinition getFieldOfUnSelfTree(IEntityDefinition system) {
		List<? extends IFieldDefinition> refFields = biz(system.getId()).fields();
		for (IFieldDefinition fld : refFields) {
			if (fld.isGroupBy()) {
				return fld;
			}
		}
	...
	@Override
	public IFieldDefinition getFieldOfSelfTree(IEntityDefinition system) {
		if (system == null) {
			return null;
		}
		List<? extends IFieldDefinition> refFields = biz(system.getId()).fields();
		...
		for (IFieldDefinition fld : getFieldsOfSystemFK(system)) {
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfSystemFK(IEntityDefinition system) {
		List<IFieldDefinition> ret = new LinkedList();
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfSystemFK(IEntityDefinition system, Class fkType) {
		List<IFieldDefinition> ret = new LinkedList();

		List<? extends IFieldDefinition> fields = getFieldsOfEnabled(system);
		if (fields != null) {
			for (IFieldDefinition field : fields) {
				if (this.isSystemFK(field)) {
	...
	protected abstract Option[] geDicOptions(IFieldDefinition field);

	public Option[] getOptions(IFieldDefinition field) {
		if (!this.isSystemFK(field)) {
	...
	protected Option[] evalOptions(String str) {
		...
		if (str.startsWith("[") && str.endsWith("]")) {
			...
			ITenantPreference config = moduleEngine.getSoftConfig(key);
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfFK(IEntityDefinition system) {
		List<IFieldDefinition> ret = new LinkedList();
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfNavi(IEntityDefinition system) {
		List<IFieldDefinition> ret = new LinkedList();
	...
	@Override
	public int getGridWidth(IFieldDefinition field) {
	...
	@Override
	public boolean isManyToOne(IFieldDefinition field) {
		IFieldDataType type = field.getType();
		return (type.isSystem() || (type.isV1Dic() && !type.isManyToMany()) || type.isV1GEO()) && !field.isSysMultiple();
	}

	@Override
	public boolean isV1Dic(IFieldDefinition field) {
		return field.getType().isV1Dic();
	}

	@Override
	public boolean isBoolean(IFieldDefinition field) {
		return field.getType().isBoolean();
	}

	@Override
	public boolean isOneToOne(IFieldDefinition field) {
		return false;
	}

	@Override
	public boolean isOneToMany(IFieldDefinition field) {
		return false;
	}

	@Override
	public boolean isManyToMany(IFieldDefinition field) {
		return field.isSysMultiple() || field.getType().isManyToMany();
	}

	@Override
	public boolean isNumber(IFieldDefinition field) {
		return field.getType().isNumber();
	}

	@Override
	public boolean isInteger(IFieldDefinition field) {
		return field.getType().isNumber();
	}

	@Override
	public boolean isRichText(IFieldDefinition field) {
		return field.getType().isRichText();
	}

	@Override
	public boolean isDate(IFieldDefinition field) {
		return field.getType().isDate();
	}

	@Override
	public boolean isText(IFieldDefinition field) {
		if (field.getType().isString()) {
			return getPrecision(field) > APIConst.INPUT_FIELD_MAX_LENGTH;
		}
	...
	@Override
	public boolean isString(IFieldDefinition field) {
		return field.getType().isString();
	}

	@Override
	public boolean isUpload(IFieldDefinition field) {
		return field.getType().isUpload();
	}

	@Override
	public boolean isImage(IFieldDefinition field) {
	...
	@Override
	public boolean isMultiUpload(IFieldDefinition field) {
		return "MultiUpload".equals(field.getType().getCode());
	}

	@Override
	public boolean isMultiImage(IFieldDefinition field) {
	...
	@Override
	public boolean isSubSystem(IFieldDefinition field) {
		String type = field.getType().getCode();
		return "SubSystem".equals(type) || "FakeSubSystem".equals(type);
	}

	@Override
	public boolean isFakeSubSystem(IFieldDefinition field) {
		String type = field.getType().getCode();
		return "FakeSubSystem".equals(type);
	}

	@Override
	public boolean isBuildin(IEntityDefinition system, String prop) {
		try {
	...
	@Override
	public boolean isGridField(IFieldDefinition field) {
	...
	@Override
	public String getMode(IFieldDefinition field, IAction action, boolean mustPriority, String defalutMode) {
		if (field == null) {
			return "";
		}
	...
	protected abstract boolean isMappingToMaster(IFieldDefinition fld);

	@Override
	public List<IFieldDefinition> getFieldsOfSlave(IEntityDefinition system) {
		List<IFieldDefinition> ret = new LinkedList();

		List<IFieldDefinition> fields = this.getFieldsOfExport(system);
		for (IFieldDefinition f : fields) {
			if (!isMappingToMaster(f)) {
				continue;
			}
			IEntityDefinition sys = f.getSystem();
	...
	public boolean isSlave(IEntityDefinition system) {
		List<? extends IFieldDefinition> fields = this.getFieldsOfSystemFK(system);
		for (IFieldDefinition f : fields) {
			if (isMappingToMaster(f)) {
				return true;
			}
	...
	public List<? extends IEntityDefinition> getSystemsOfSlave(IEntityDefinition system) {
		...
		List<IFieldDefinition> fields = this.getFieldsOfSlave(system);
		for (IFieldDefinition f : fields) {
			ret.add(f.getSystem());
		}
	...
	@Override
	public List<IFieldDefinition> getFieldsOfExport(IEntityDefinition system) {
		List<IFieldDefinition> ret = new ArrayList();
		// 哪些字段引用了该系统
		List<? extends IFieldDefinition> fields = biz(system.getId()).fieldsOfExport();
		if (fields != null) {
			for (IFieldDefinition field : fields) {
	...
	@Override
	public List<? extends IFieldDefinition> getFieldsOfGrid(IEntityDefinition system, String fields) {
		List<? extends IFieldDefinition> list = this.getFieldsOfEnabled(system);
		SortUtils.sort(list, F_GRID_ORDER, true);

		List<IFieldDefinition> ret = new LinkedList();
		List<IFieldDefinition> other = new LinkedList();
	...
	@Override
	public List<? extends IFieldDefinition> getFields(IEntityDefinition system) {
		if (system == null)
	...
	public List<? extends IEntityAction> getActions(IEntityDefinition system) {
		if (system == null)
			return new LinkedList();
	...
	@Override
	public List<? extends IFieldDefinition> getFields(IFieldGroup group) {
		List<? extends IFieldDefinition> list = biz(group.getSystem().getId()).fields();
		List<IFieldDefinition> ret = new LinkedList();
		for (IFieldDefinition f : list) {
	...
	@Override
	public String getPropName(IFieldDefinition f) {
		if (f == null)
			return null;
	...
	public String getTargetClassName(IFieldDefinition field) {
	 ...
	 * @return 字段类简称
	 */
	public String getClassName(IFieldDefinition field) {
		String name = "";
	 ...
	 * @return
	 */
	public String getClassName(IEntityDefinition system) {
		return getPackageOfAutoSystem(system) + "." + getSimpleClassName(system);
	}

	public Class getGenericType(IFieldDefinition field) throws DemsyException {
	...
	@Override
	public Class getType(IFieldDefinition field) throws DemsyException {
	...
	@Override
	public Class getType(IEntityDefinition system) throws DemsyException {
	...
	public String getVersion(IEntityDefinition system) {
	...
	public synchronized List<String> compileSystem(IEntityDefinition system) throws DemsyException {
		return compiler.compileSystem(system, false);
	}

	public String getTableName(IEntityDefinition system) {
		return system.getMappingTable();
	}

	public Map<String, IFieldDefinition> getFieldsMap(IEntityDefinition system) {
		return this.getFieldsMap(this.getFieldsOfEnabled(system));
	}

	public Map<String, IFieldDefinition> getFieldsMap(List<? extends IFieldDefinition> list) {
		Map<String, IFieldDefinition> map = new HashMap();
		for (IFieldDefinition fld : list) {
	...
	@Override
	public IEntityDefinition getSystem(Long systemID) {
	...
	public List<? extends IEntityDefinition> getSystems(ITenant soft) {
		return orm().query(entityDefEngine.getStaticType(BIZSYS_BZUDF_SYSTEM), Expr.eq(F_SOFT_ID, soft.getId()));
	}
	...
	public void loadFieldValue(Object obj, IEntityDefinition system) {
		if (log.isTraceEnabled())
			log.tracef("加载业务数据... [%s] %s", system, JSON.toJson(obj));

		List<? extends IFieldDefinition> fks = this.getFieldsOfSystemFK(system);
		for (IFieldDefinition fk : fks) {
			try {
		...
		for (IFieldDefinition fk : this.getFieldsOfEnabled(system)) {
			try {
	...
	public String[] getCascadeMode(IFieldDefinition field, Object obj) {
		...
		for (String str : strs) {
			...
			if ("*".equals(cascadeList)) {
				...
				try {
					...
					IFieldDefinition parentField = this.getField(field.getSystem(), cascadeField);
					String code = "";
					if (parentValue == null) {
	...
	private Object getFieldValue(Object obj, IFieldDefinition field) {
		return Obj.getValue(obj, getPropName(field));
	}

	private Object loadFieldValue(Object obj, IFieldDefinition field) throws DemsyException {
	...
	private Object loadFieldDefaultValue(Object obj, IFieldDefinition field) throws DemsyException {
		String fldname = this.getPropName(field);
		Object fldvalue = Obj.getValue(obj, fldname);
		if (fldvalue != null)
			return fldvalue;
	...
	public void validate(IEntityDefinition system, IAction action, Object data, Map<String, String> fieldMode) throws DemsyException {
		...
		if (props.size() > 0) {
			Map<String, IFieldDefinition> map = this.getFieldsMap(this.getFieldsOfEnabled(system));
			StringBuffer sb = new StringBuffer();
	...
	private IFieldDefinition getField(IEntityDefinition system, String prop) {
		return this.getFieldsMap(this.getFieldsOfEnabled(system)).get(prop);
	}

	@Override
	public Nodes makeOptionNodes(IFieldDefinition field, String mode, Object data, String idField) {
		...
		try {
			if (("E".equals(mode) || "M".equals(mode))) {
				...
				if (root.is((byte) 1) && entityDefEngine.isSystemFK(field)) {
				...
				Object value = getFieldValue(data, field);
				if (value != null) {
					if (entityDefEngine.isSystemFK(field)) {
						value = loadFieldValue(data, field);
						if (value != null) {
						...
						Option[] options = entityDefEngine.getOptions(field);
	...
	private Node makeNode(Nodes maker, Object obj, String parentNode, String prefix, String paramPrefix, boolean selectable, String idField) {
		...
		Integer ord = Obj.getValue(obj, APIConst.F_ORDER_BY);
		item.setOrder(ord);

		return item;
	}

	public List<String> makeCascadeExpr(Object obj, IFieldDefinition field, String mode) {
		if (obj == null) {
			return null;
		}
	...
	private void makeFieldOptions(Nodes root, Node node, Object obj, IFieldDefinition field, String mode, String paramPrefix, String idField) throws DemsyException {
		...
		if (isSystemFK(field)) {
			...
			String fkField = entityDefEngine.getPropName(field);
			Class refType = entityDefEngine.getType(refSys);
			...
			// 计算数据分组、自身分组
			IFieldDefinition groupByFld = getFieldOfUnSelfTree(refSys);
			if (groupByFld != null) {
				// if (node != null) {
				...
				IFieldDefinition groupTreeFld = getFieldOfSelfTree(groupByFld.getRefrenceSystem());
				if (groupTreeFld != null) {
					groupTree = getPropName(groupTreeFld);
				}
			}
			IFieldDefinition selfTreeFld = getFieldOfSelfTree(refSys);
			if (selfTreeFld != null) {
	...
	@Override
	public Nodes makeNaviNodes(IEntityDefinition entityDefinition, String idField, boolean removeSelfLeaf) {
		log.debugf("计算业务系统导航菜单数据......[system=%s]", entityDefinition);

		Nodes root = Nodes.make();

		List<? extends IFieldDefinition> fkFields = this.getFieldsOfNavi(entityDefinition);
		IFieldDefinition selfTreeFld = getFieldOfSelfTree(entityDefinition);

		for (IFieldDefinition fld : fkFields) {
		...
		log.debugf("计算业务系统导航菜单数据: 结束. [system=%s]", entityDefinition);
	...
	private <T> List<T> setupFromJson(IOrm orm, ITenant soft, Class<T> klass, String idFld, String jsonData) {
	...
	private List saveObj(IOrm orm, ITenant soft, Object newObj, CndExpr expr, String idFld) {
		List ret = new LinkedList();
	...
	protected int importFromJson(ITenant soft, IOrm orm, File ffolder, boolean includeFK) {
		int len = 0;
		...
		for (File file : files) {
			...
			try {
				...
				String json = com.cocit.lang.Files.readAll(is, "UTF-8");
	...
	public IFieldDefinition getField(Long fieldID) {
		return (IFieldDefinition) orm().load(this.getStaticType(BIZSYS_BZUDF_FIELD), fieldID);
	}

	@Override
	public IEntityDefinition getSystem(String systemCode) {
		return (IEntityDefinition) orm().load(this.getStaticType(BIZSYS_BZUDF_SYSTEM), Expr.eq(F_CODE, systemCode).or(Expr.eq(F_GUID, systemCode)));
	}
