...
import static com.cocit.api.APIConst.BIZSYS_DEMSY_LIB_UIMODEL;
...
import static com.cocit.api.APIConst.F_PARENT;
import static com.cocit.api.APIConst.F_SOFT_ID;
import static com.cocit.api.APIConst.F_USAGE;
import static com.cocit.mvc.MvcConst.MvcUtil.contextPath;
import static com.cocit.mvc.MvcConst.MvcUtil.globalVariables;
import static com.cocit.mvc.MvcConst.MvcUtil.initGlobalVariables;
...
import com.cocit.api.IUIEngine;
...
import com.cocit.api.entitydef.field.CssPosition;
import com.cocit.api.entitydef.field.Dataset;
...
import com.cocit.api.webdef.IStyle.SimpleStyle;
...
import com.cocit.mvc.ui.widget.UIAccordion;
import com.cocit.mvc.ui.widget.UIBizForm;
...
import com.cocit.mvc.ui.widget.UIWidget;
import com.cocit.mvc.ui.widget.UIBizGrid.UIGridFld;
import com.cocit.mvc.ui.widget.field.UIBizFld;
import com.cocit.mvc.ui.widget.field.UIBoolFld;
import com.cocit.mvc.ui.widget.field.UIDateFld;
import com.cocit.mvc.ui.widget.field.UIDicFld;
import com.cocit.mvc.ui.widget.field.UIDicManyFld;
import com.cocit.mvc.ui.widget.field.UIGroupFld;
import com.cocit.mvc.ui.widget.field.UINumFld;
import com.cocit.mvc.ui.widget.field.UIRichTextFld;
import com.cocit.mvc.ui.widget.field.UIStrFld;
import com.cocit.mvc.ui.widget.field.UISubSystemFld;
import com.cocit.mvc.ui.widget.field.UISysFld;
import com.cocit.mvc.ui.widget.field.UISysManyFld;
import com.cocit.mvc.ui.widget.field.UITextFld;
import com.cocit.mvc.ui.widget.field.UIUploadFld;
...
public class UiEngine implements IUIEngine, MvcConst {
	...
	public UIBizGridModel makeSystemGridView(IModule mdl, String colNames, String idField, boolean existNaviTree) throws DemsyException {
		...
		if (grid == null) {
			...
			IEntityDefinition sys = moduleEngine.getSystem(mdl);
			List<? extends IFieldDefinition> flds = entityDefEngine.getFieldsOfGrid(sys, colNames);
			int count = 1;
			int width = 0;
			...
			for (IFieldDefinition fld : flds) {
				...
				if (entityDefEngine.isString(fld) || entityDefEngine.isRichText(fld) || entityDefEngine.isSystemFK(fld)) {
					col.setString(true);
				}

				col.setLabel(fld.getName());
				col.setName(fld.getCode());
				col.setPropName(entityDefEngine.getPropName(fld));
				...
				IFieldDefinition field = fld.getRefrenceField();
				if (field == null) {
					field = fld;
				}
				...
				if (entityDefEngine.isNumber(field) && options.length == 0) {
					col.setAlign(Align.RIGHT);
					col.setSearchType("number");
				} else if (entityDefEngine.isBoolean(field)) {
					col.setAlign(Align.CENTER);
				} else if (entityDefEngine.isDate(field)) {
					col.setSearchType("date");
				}

				// 计算字段宽度
				int w = entityDefEngine.getGridWidth(field);
				col.setWidth(w);
				width += w;
	...
	public UIBizNaviModel makeSystemNaviView(IModule mdl) throws DemsyException {
		...
		if (navi == null) {
			...
			if (entityDefEngine.getFieldsOfNavi(moduleEngine.getSystem(mdl)).size() == 0) {
				navi.setWidth(0);
			} else {
				navi.setWidth(300);
	...
	public UIBizFormModel makeSystemFormView(IModule module, IAction action, Object data) throws DemsyException {
		...
		IEntityDefinition sys = moduleEngine.getSystem(module);

		// 获取运行时自定义字段
		List<IFieldDefinition> customFields = new LinkedList();
		Map<String, IFieldDefinition> customFieldsMap = new HashMap();
		List<IFieldDefinition> fkFieldsOfRuntimeCustom = (List<IFieldDefinition>) entityDefEngine.getFieldsOfSystemFK(sys, IRuntimeField.class);
		List<String> fkPropsOfRuntimeCustom = new ArrayList();
		for (IFieldDefinition fld : fkFieldsOfRuntimeCustom) {
			String propname = entityDefEngine.getPropName(fld);
			fkPropsOfRuntimeCustom.add(propname);
			IRuntimeField custom = Obj.getValue(data, propname);
			List<? extends IFieldDefinition> flds = entityDefEngine.makeFields(custom);
			if (flds != null) {
				for (IFieldDefinition f : flds) {
					customFields.add(f);
		...
		List<? extends IFieldGroup> bizGroups = entityDefEngine.getFieldGroups(sys);
		...
		for (IFieldGroup bizGroup : bizGroups) {
			...
			uiGroup.setMode(entityDefEngine.getMode(bizGroup, action));

			List<? extends IFieldDefinition> groupFields = entityDefEngine.getFieldsOfEnabled(bizGroup);
			for (IFieldDefinition bzField : groupFields) {
				try {

					// 用运行时自定义字段覆盖物理字段
					String bzFieldPropName = entityDefEngine.getPropName(bzField);
					IFieldDefinition customField = customFieldsMap.get(bzFieldPropName);
					if (customField != null) {
					...
					UIBizFld uiField = this.convertFld(module, form, bzField, action, null, data, String.class.equals(entityDefEngine.getType(bzField)) ? F_GUID : F_ID);
					if (uiField != null) {
		...
		uiGroup.setMode("S");
		Map<Long, IFieldDataType> types = entityDefEngine.getFieldTypesById();
		long dynId = System.currentTimeMillis();
		for (IFieldDefinition bzField : customFields) {
			try {
	...
	private UIBizFld convertFld(IModule module, UIBizForm form, IFieldDefinition bzField, IAction action, String defaultMode, Object data, String idField) {

		String[] cascade = entityDefEngine.getCascadeMode(bzField, data);
		String mode = entityDefEngine.getMode(bzField, action, false, defaultMode);
		if (entityDefEngine.getModeValue(mode) < entityDefEngine.getModeValue(cascade[1])) {
			mode = entityDefEngine.getUiMode(cascade[1]);
		}
		// log.tracef("bzField:%s, mode:%s, defaultMode:%s", bzField.getName(),
		// mode, defaultMode);

		String prop = entityDefEngine.getPropName(bzField);
		...
		String namePost = "";
		if (entityDefEngine.isV1Dic(bzField)) {// 遗留系统字典
			namePost = ".id";
		}

		Class bzFieldType = entityDefEngine.getType(bzField);
		// ui model
		if (entityDefEngine.isBoolean(bzField)) {// boolean
			uiField = new UIBoolFld(null, bzField.getId()).setOptionNode(entityDefEngine.makeOptionNodes(bzField, mode, data, idField));
		} else if (entityDefEngine.isUpload(bzField)) {// upload
			...
			isUpload = true;
		} else if (entityDefEngine.isRichText(bzField)) {// richtext
			uiField = new UIRichTextFld(null, bzField.getId());
			isRichText = true;
		} else if (entityDefEngine.isText(bzField)) {// text
			uiField = new UITextFld(null, bzField.getId());
		} else if (entityDefEngine.getOptions(bzField).length > 0) {// auto dic
			if (entityDefEngine.isManyToMany(bzField)) {
				uiField = new UIDicManyFld(null, bzField.getId()).setOptionNode(entityDefEngine.makeOptionNodes(bzField, mode, data, idField));
				form.setHasMultiSelect(true);
			} else {
				uiField = new UIDicFld(null, bzField.getId()).setOptionNode(entityDefEngine.makeOptionNodes(bzField, mode, data, idField));
			}
		} else if (entityDefEngine.isDate(bzField)) {// date
			uiField = new UIDateFld(null, bzField.getId());
			isDate = true;
		} else if (entityDefEngine.isNumber(bzField)) {// number
			uiField = new UINumFld(null, bzField.getId());
		} else if (entityDefEngine.isString(bzField)) {// string
			uiField = new UIStrFld(null, bzField.getId());
			if (bzField.isPassword()) {
				((UIStrFld) uiField).setInputType("password");
			}
		} else if (entityDefEngine.isSystemFK(bzField)) {// system refrence
			Class type = null;
			if (entityDefEngine.isManyToMany(bzField)) {
				namePost = ".id";
				...
				try {
					type = Cls.getType(entityDefEngine.getType(bzField.getSystem()), entityDefEngine.getPropName(bzField));
					if (type.isAssignableFrom(entityDefEngine.getType(bzField.getRefrenceSystem()))) {
						namePost = ".id";
					}
				} catch (Throwable e) {
			...
			Nodes options = entityDefEngine.makeOptionNodes(bzField, mode, data, idField);
			uiField.setOptionNode(options);
			if (options.is((byte) 1)) {
				isCombobox = true;
				IModule refModule = moduleEngine.getModule(module.getTenantGuid(), bzField.getRefrenceSystem());
				List<String> rules = entityDefEngine.makeCascadeExpr(data, bzField, action.getMode());
				...
				if (type.equals(String.class)) {
					uiField.setKeyProp(APIConst.F_GUID);
					url += "&idField=" + APIConst.F_GUID;
				}
			...
			uiField = new UIStrFld(null, bzField.getId());
		} else if (entityDefEngine.isFieldRef(bzField)) {// iglore field refrence
			return null;
		} else if (entityDefEngine.isSubSystem(bzField)) {
			...
			if (entityDefEngine.isFakeSubSystem(bzField)) {
				uiSubSysFld.setFake(true);
			}
			if (entityDefEngine.isMultiUpload(bzField)) {
				CocField ann = (CocField) bzFieldType.getAnnotation(CocField.class);
				refSystem = entityDefEngine.getSystem(ann.fkTable());
				refProps = Str.toArray(ann.refrenceFields());
			}

			Map<String, IFieldDefinition> refFields = entityDefEngine.getFieldsMap(refSystem);
			for (String refProp : refProps) {
				IFieldDefinition refFld = refFields.get(refProp);
				if (refFld != null) {
					UIBizFld uiRefFld = this.convertFld(module, form, refFld, action, defaultMode, null, null);

					uiRefFld.setMode("E");
					if (entityDefEngine.isNumber(refFld) && !entityDefEngine.isInteger(refFld)) {
						uiRefFld.setPattern("#.00");
					}
					if (entityDefEngine.isSystemFK(refFld))
	...
	private void convertSubFields(IModule module, UIBizForm form, IAction action, Object data, IFieldDefinition bzfld, UIBizFld uifld) {
		Class proptype = entityDefEngine.getType(bzfld);
		...
		List<IFieldDefinition> bzsubflds = new LinkedList();
		...
		long count = 1;
		Class systype = entityDefEngine.getType(bzfld.getSystem());
		...
		if (subanns != null && subanns.length > 0) {
			for (CocField2 ann : subanns) {
				IFieldDefinition fld = ((BizEngine) entityDefEngine).parseBizField(module.getTenantGuid(), propme, ann.property(), bzfld.getSystem(), null);
				fld.setId(count++);
			...
			for (Field f : fields) {
				IFieldDefinition fld = ((BizEngine) entityDefEngine).parseBizField(module.getTenantGuid(), propme, f.getName(), bzfld.getSystem(), null);
				fld.setId(count++);
				fld.setPropName(prop + "." + fld.getPropName());
		...
		SortUtils.sort(bzsubflds, APIConst.F_ORDER_BY, true);
		Map<String, UIBizFld> subMap = new HashMap();
		for (IFieldDefinition bzChild : bzsubflds) {
			UIBizFld subUiFld = this.convertFld(module, form, bzChild, action, uifld.getMode(), data, APIConst.F_GUID);
	...
	public IPage loadIndexPage() {
		return (IPage) Demsy.orm().load(entityDefEngine.getStaticType(BIZSYS_UIUDF_PAGE), Expr.eq(F_USAGE, IPage.USAGE_IDX).and(Expr.eq(F_SOFT_ID, Demsy.me().getSoft())).setFieldRexpr("id", true));
	}
	...
	public IPageBlock loadPageBlock(Long blockID) {
		...
		if (block == null) {
			return (IPageBlock) Demsy.orm().load(entityDefEngine.getStaticType(BIZSYS_UIUDF_PAGE_BLOCK), Expr.eq(F_ID, blockID));
		}
		return block;
	}
	...
	private void loadViewComponents() {
		if (this.viewComponentList.size() == 0 || !Demsy.appconfig.isProductMode()) {
			viewComponentList = Demsy.orm().query(entityDefEngine.getStaticType(BIZSYS_DEMSY_LIB_UIMODEL), Expr.asc(F_ORDER_BY));
	...
	public UIPageView makePageView(IPage page, IPageBlock dynamicBlock, Long dynamicModuleID, Long dynamicDataID) {
		...
		if (dynamicModuleID != null && dynamicModuleID > 0) {
			if (dynamicDataID != null && dynamicDataID >= 0) {
				...
				pathData = Demsy.orm().load(entityDefEngine.getType(pathSystem), Expr.eq(F_ID, dynamicDataID));
			}
		}
	...
	private UIBlockViewModel makeBlockViewWithChilren(Map<Long, UIBlockViewModel> cachedBlockViews, Map<Long, IPageBlock> cachedBlocks, IPageBlock defaultParentBlock, IPageBlock pageBlock, Long dynamicModuleID, Long dynamicDataID,
		...
		List<IPageBlock> children = Demsy.orm().query(entityDefEngine.getStaticType(BIZSYS_UIUDF_PAGE_BLOCK), Expr.eq(F_PARENT, pageBlock).and(Expr.eq(F_DISABLED, false)).addAsc(F_ORDER_BY));
	...
	private void addViewComponent(Node fnode, IUIViewComponent viewType, IFieldDefinition field, String var) {
		...
		if (entityDefEngine.isImage(field)) {
		...
		} else if (entityDefEngine.isNumber(field) && !entityDefEngine.isInteger(field)) {
		...
		if (blockID != null) {
			...
			if (blockContext != null) {
				...
				if (catalogSystem != null) {
					...
					List<? extends IFieldDefinition> fields = entityDefEngine.getFieldsOfEnabled(catalogSystem);
				...
				if (system != null) {
					...
					List<? extends IFieldDefinition> fields = entityDefEngine.getFieldsOfEnabled(system);
					for (IFieldDefinition f : fields) {
	...
	private void loadUiStyle() {
		if (this.styleCache.size() == 0 || !appconfig.isProductMode()) {
			List<IStyle> uiStyleList = Demsy.orm().query(entityDefEngine.getStaticType(BIZSYS_UIUDF_STYLE), Expr.asc(F_ORDER_BY));
