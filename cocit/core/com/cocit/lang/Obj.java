...
import com.cocit.api.entity.IDynaEntity;
...
public abstract class Obj {
	...
	public static String toString(Object obj) {
		...
		if (obj instanceof IDataEntity)
			return Cls.getType(obj.getClass()).getSimpleName() + "#" + ((IDataEntity) obj).getId();
		else
	...
	public static <T> T getValue(Object obj, final String path) {
		...
		if (dot > -1) {
			...
			try {
				...
				try {
					if (obj instanceof IDynaEntity) {
						return (T) ((IDynaEntity) obj).get(path);
					}
		...
		try {
			...
			if (Obj.isEntity(value) && !Cls.isEntityType(type)) {
				if (Number.class.isAssignableFrom(type)) {
					...
					value = getValue(value, APIConst.F_GUID);
				}
			}
