...
public class ObjcetNaviNode {
		...
		for (Entry<String, ObjcetNaviNode> entry : child.entrySet()) {
			...
			if (onn.isLeaf()) {
				try {
					...
					if (m == null) {
						if (obj instanceof IDynaEntity && values != null && values.length > 0) {
							((IDynaEntity) obj).set(key, values[0]);
						}
					} else {
						throw e;
					}
			...
			try {
				...
				if (obj instanceof IDynaEntity) {
					onn.injectDynamic((IDynaEntity) obj, key);
				}
				continue;
			}
		}
	...
	private Object injectDynamic(IDynaEntity obj, String path) {
		for (Entry<String, ObjcetNaviNode> entry : child.entrySet()) {
			ObjcetNaviNode onn = entry.getValue();
