...
import static com.cocit.api.APIConst.F_CREATED;
import static com.cocit.api.APIConst.F_ORDER_BY;
import static com.cocit.mvc.MvcConst.URL_UI;
...
public class UIBlockContext {
	...
	// 栏目字段
	private IFieldDefinition catalogField;
	...
	private void parseDataset() {
		...
		type = entityDefEngine.getType(system);
		...
		// 解析栏目模块
		Map<String, IFieldDefinition> fldmap = entityDefEngine.getFieldsMap(system);
		for (String srcRule : srcRuleArray) {
			...
			if (!Str.isEmpty(fkfldName)) {
				...
				// 栏目模块解析成功
				if (catalogField != null && entityDefEngine.isSystemFK(catalogField)) {
					...
					catalogType = entityDefEngine.getType(catalogSystem);
		...
		try {
			...
			if (isFullDynaCatalogModule) {
				...
				IEntityDefinition refSystem = moduleEngine.getSystem(catalogModule);
				Class refType = entityDefEngine.getType(refSystem);
				for (IFieldDefinition fld : fields) {
					if (entityDefEngine.getType(fld).equals(refType)) {
						srcRuleArray.add(entityDefEngine.getPropName(fld) + ".id eq " + catalogModule.getId());
			...
			if (this.isInherit() && parent.module != null && parent.itemObjs != null && parent.itemObjs.size() > 0) {
				List<? extends IFieldDefinition> fields = entityDefEngine.getFieldsOfEnabled(system);
				IEntityDefinition parentSystem = moduleEngine.getSystem(parent.module);
				Class parentType = entityDefEngine.getType(parentSystem);
				for (IFieldDefinition fld : fields) {
					if (entityDefEngine.getType(fld).equals(parentType)) {

						for (Object parentItem : parent.itemObjs) {
							srcRuleArray.add(entityDefEngine.getPropName(fld) + ".id eq " + Obj.getId(parentItem));
						}
			...
			for (String srcRule : srcRuleArray) {
				...
				// 是外键字段且没有被解析过
				if (fkFld != null && entityDefEngine.isSystemFK(fkFld)) {
					// 外键字段类型
					Class fldClass = entityDefEngine.getType(fkFld);
	...
	private String getImageField(IEntityDefinition system) {
		IFieldDataType fldlib = entityDefEngine.getFieldTypes().get("Upload");
		for (IFieldDefinition f : entityDefEngine.getFieldsOfEnabled(system)) {
			...
			if (fileType != null && (fileType.indexOf(".jpeg") > -1 || fileType.indexOf(".jpg") > 0 || fileType.indexOf(".gif") > 0) || fileType.indexOf(".png") > 0) {
				String imgFld = entityDefEngine.getPropName(f);
	...
	public IFieldDefinition getCatalogField() {
		return catalogField;
	}
