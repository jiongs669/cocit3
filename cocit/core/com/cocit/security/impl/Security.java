package com.cocit.security.impl;
...
import com.cocit.api.security.IPermission;
import com.cocit.api.security.ITenant;
import com.cocit.api.security.IUser;
import com.cocit.engine.RootUserFactory;
...
public class Security implements ISecurity {
	...
	public IUser checkUser(ITenant soft, String realmCode, String username, String pwd) throws SecurityException {
		...
		if (Str.isEmpty(realmCode)) {
			...
			if (user == null) {
				user = (IUser) orm().load(entityDefEngine.getStaticType(BIZSYS_DEMSY_SOFT), Expr.eq(F_CODE, username));
			}
		} else {
	...
	private Map<Long, List<PermissionItem>> loadPermissions(Long softID) {
		...
		IEntityDefinition sys = entityDefEngine.getSystem(APIConst.BIZSYS_ADMIN_PERMISSION);
		Class type = entityDefEngine.getType(sys);
