...
public abstract class WebUtils {
	...
	/**
	 * Set a system properties to the web application root directory. The key of
	 * the system properties can be defined with the "webAppRootKey" context-param
	 ...
	 *             if the system properties is already set, or if the WAR file is
	 *             not expanded
	...
	public static void setWebAppRootSystemProperty(ServletContext servletContext) throws IllegalStateException {
		...
		if (root == null) {
			throw new IllegalStateException("Cannot set web app root system properties when WAR file is not expanded");
		}
		String param = servletContext.getInitParameter(WEB_APP_ROOT_KEY_PARAM);
		String key = (param != null ? param : DEFAULT_WEB_APP_ROOT_KEY);
		String oldValue = System.getProperty(key);
		if (oldValue != null && !Str.pathEquals(oldValue, root)) {
			throw new IllegalStateException("Web app root system properties already set to different value: '" + key + "' = [" + oldValue + "] instead of [" + root + "] - "
					+ "Choose unique values for the 'webAppRootKey' context-param in your web.xml files!");
		}
		System.setProperty(key, root);
		servletContext.log("Set web app root system properties: '" + key + "' = [" + root + "]");
	}

	/**
	 * Remove the system properties that points to the web app root directory. To
	...
	public static Boolean getDefaultHtmlEscape(ServletContext servletContext) {
		...
		Assert.notNull(servletContext, "ServletContext must not be null");
		String param = servletContext.getInitParameter(HTML_ESCAPE_CONTEXT_PARAM);
		return (Str.hasText(param) ? Boolean.valueOf(param) : null);
	}
