/*
 * @(#)MailBasisBean.java 1.00 2006/04/21 Copyright (c) 2006, Stephan Sann
 * 21.04.2006 ssann Vers. 1.0 created
 */

package com.jiongsoft.cocit.modules.webmail.model;
 ...
 * @version 1.0
 */
public class MailBasisModel implements Lifecycle, Serializable {
	 ...
	 *            Hochgeladenes File.
	 */
	private void addAttachment(UploadModel upFile) throws Exception {
		...
		if ((upFile.getName() != null) && (upFile.getName().length() >= 1)) {
			...
			if (upFile.getContentType().toLowerCase().startsWith("text")) {
				...
				mbp.setContent(new String(upFile.getBytes()), upFile.getContentType());
			}
			...
			// Sonst Content als byte-Array eintueten.
				...
				mbp.setDataHandler(new DataHandler(new ByteArrayDataSource(upFile.getBytes(),
	 ...
	 *            Hochgeladenes File.
	 */
	public void setNewAttachment(UploadModel uploadedFile) {
	...
	/**
	 * Liefert ein (nicht vorhandenes) Property "newAttachment". Noetig, damit
	 * wir dieses in der JSP verwenden koennen.
	 */
	public UploadModel getNewAttachment() {
