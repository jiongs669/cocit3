...
/**
 * 该类在 {@link org.nutz.mvc.impl.UrlMappingImpl} 的基础上进行修改，详细参见 @DEMSY 标识部分
 * 
 * @author yongshan.ji
 * 
 */
public class DemsyUrlMappingImpl implements UrlMapping {

	private static final Log log = Logs.getLog(DemsyUrlMappingImpl.class);

	private Map<String, ActionInvoker> map;

	private MappingNode<ActionInvoker> root;

	public DemsyUrlMappingImpl() {
		...
		for (String path : ai.getPaths()) {
			...
			if (log.isDebugEnabled()) {
				...
				if (null != method)
					...
					str = "???";
				log.debugf("%s >> %s | @Ok(%s) @Fail(%s) | by %d Filters | (I:%s/O:%s)", Strings.alignLeft(sb, 30, ' '), str, ai.getOkView(), ai.getFailView(),
