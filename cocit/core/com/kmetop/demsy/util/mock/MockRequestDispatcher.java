...
public class MockRequestDispatcher implements RequestDispatcher {
	...
	public void forward(ServletRequest request, ServletResponse response) {
		...
		Assert.notNull(response, "CKFinder must not be null");
		...
		if (logger.isDebugEnabled()) {
			logger.debug("MockRequestDispatcher: forwarding to URL ["
					+ this.url + "]");
		}
	}
	...
	protected MockHttpServletResponse getMockHttpServletResponse(
		...
		if (response instanceof HttpServletResponseWrapper) {
			return getMockHttpServletResponse(((HttpServletResponseWrapper) response)
					.getResponse());
		}
