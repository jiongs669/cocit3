(function($) {
	...
	AdGallery.prototype = {
		...
		setupElements : function() {
			...
			this.thumbs_wrapper.hover(function() {
				// context.slideshow.stop();
			}, function() {
				// context.slideshow.start();
			});
			this.thumbs_wrapper.bind("mousewheel", function() {
				context.slideshow.stop();
				var thumbs_wrapper = $(this);
				var delta = 0;
				...
				var top = thumbs_wrapper.scrollTop() - delta;
		...
		findImages : function() {
			...
			thumbs.each(function(i) {
				...
				var author = false;
				if (thumb.attr("author") && thumb.attr("author").length) {
					author = thumb.attr("author");
				}
				var id = false;
				if (thumb.attr("id") && thumb.attr("id").length) {
					id = thumb.attr("id");
				}
				context.images[i] = {
		...
		initBackAndForward : function() {
			...
			// this.scroll_forward.append( $("<div
			// class=\"ad-forward-logo\"></div>"));
			// this.scroll_back.append( $("<div
			// class=\"ad-back-logo\"></div>"));
			var has_scrolled = 0;
			var thumbs_scroll_interval = false;
			$(this.scroll_back).add(this.scroll_forward).click(function() {
				var height = context.nav_display_height - 44;
				...
				thumbs_scroll_interval = setInterval(function() {
					has_scrolled++;
					// Don't want to stop the slideshow just because we scrolled
					// a pixel or two
		...
		_getDescription : function(image) {
			var desc = false;
			if (image.desc.length || image.title.length || image.author.length) {
				var title = "";
				if (image.title.length) {
					title = "<div class=\"ad-description-title\">" + image.title + "</div>";
				}
				var author = "";
				if (image.author.length) {
					author = "<div class=\"ad-author-title\">" + image.author + "</div>";
				}
				var desc = "";
				if (image.desc.length) {
					desc = "<div>" + image.desc + "</div>";
				}
				desc = $("<div class=\"ad-image-description\">" + title + author + desc + "</div>");
			}
			return desc;
		},
		...
		_showWhenLoaded : function(index, callback) {
			if (this.images[index]) {
				...
				if (desc) {
					...
					var width = size.width - parseInt(desc.css("margin-left"), 10) - parseInt(desc.css("margin-right"), 10);
